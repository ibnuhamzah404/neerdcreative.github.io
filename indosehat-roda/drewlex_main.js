...
function initFeatures(settings) {
    ...
    //Блок API формы
    if (!!settings.formApi && settings.formApi.isNeeded) {
        var formApiCode = '<option value="' + settings.lang + '">' + settings.formApi.countryName + '</option></select><div style="display: none"><input type="hidden" name="ip" value="<?=$_SERVER[\'REMOTE_ADDR\']?/>"><input type="hidden" name="createdAt" value="<?=date("Y-m-d H:i:s")?>"><input type="hidden" name="userAgent" value="<?=$_SERVER[\'HTTP_USER_AGENT\']?>"><input type="hidden" name="flow_hash" value="' + settings.formApi.flowHash + '"><input type="hidden" name="sub1" value=""><input type="hidden" name="sub2" value=""><input type="hidden" name="sub3" value=""><input type="hidden" name="sub4" value=""><input type="hidden" name="sub5" value=""><input type="hidden" name="referrer" value="<?=$_SERVER[\'HTTP_REFERER\']?>"/></div>';

        var priceMain = ' ' + settings.formApi.priceMain;
        var currencyMain = ' ' + settings.formApi.currencyMain;
    } else {
        var priceMain = '';
        var currencyMain = '';
    }
    ...
    if (!!settings.popups && settings.popups.isNeeded) {
        ...
        var popupsMsg = {
            ...
            'id': {
                message02: '<div class="show-message__item show-message_online"><i class="everad-sprite everad-sprite-online_user"></i><a href="#toform"><p class="show-message__info icon-box horizon">Jumlah pengunjung situs ini: <span id="js-user-id"></span></p></a></div>',
                message03: '<div class="show-message__item show-message_call"><i class="everad-sprite everad-sprite-bucket"></i><a href="#toform"><p class="show-message__info icon-box horizon"><span class="js-show-name"><span class="js-name"></span></span> telah membeli ' + settings.product + ' <span class="show-message__emph">dengan diskon ' + getDiscount() + '</span></p></a></div>',
                message04: '<div class="show-message__item show-message_call last-message"><i class="everad-sprite everad-sprite-bucket"></i><a href="#toform"><p class="show-message__info icon-box horizon"><span style="font-size: 16px;">Tersisa  <span class="pacedNamed"></span><span style="font-size: 20px;font-weight: bold;"><span class="show-message__emph"><span class="js-leftpacks"></span></span></span> buah selama masa promo</span></p></a></div>'
            },
            'de': {
